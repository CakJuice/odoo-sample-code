...
class KaHrEmployeePromote(models.Model):
    ...
    is_contract = fields.Boolean(string="Kontrak", default=False, readonly=True,
        states={'draft': [('readonly', False)]})
    date_end = fields.Date(string="Tanggal Akhir", readonly=True,
        states={'draft': [('readonly', False)]})
    department_id = fields.Many2one('hr.department', string="Divisi Baru", readonly=True,
        states={'draft': [('readonly', False)]})
    jabatan_id = fields.Many2one('hr.jabatan', string="Jabatan Baru", readonly=True,
        states={'draft': [('readonly', False)]})
    pangkat_id = fields.Many2one('hr.pangkat', string="Pangkat Baru", readonly=True,
        states={'draft': [('readonly', False)]})
    golongan_id = fields.Many2one('hr.golongan', string="Golongan Baru", readonly=True,
        states={'draft': [('readonly', False)]})
    status_id = fields.Many2one('hr.status', string="Status Baru", readonly=True,
        states={'draft': [('readonly', False)]})
    company_id = fields.Many2one('res.company', string="Unit/PG Baru", readonly=True,
    ...
    # date_done = fields.Datetime(string="Tanggal Selesai", readonly=True,
    #     states={'processed': [('readonly', False)]})
    ...
    def create(self, vals):
        ...
        if promote_count > 0:
            raise ValidationError("Ada data history promosi yang belum diproses!. Anda tidak dapat melanjutkan aksi ini.")

        record = super(KaHrEmployeePromote, self).create(vals)
    ...
    def unlink(self):
        ...
        return super(KaHrEmployeePromote, self).unlink()
